...
func (lb *LeaderBoard) GetLeader(c *gin.Context) {
	...
	query := "select id, name, played, wins, losses, round((score + 1) / 3 * 200 - 100, 2) as score from leaderboard order by score desc, wins desc"
	if length > 0 {
...
func (lb *LeaderBoard) PostResult(c *gin.Context) {
	...
	gamestmt, err := lb.db.Prepare("insert into games_users (game, player, position, numplayers) values (?,?,?,?);")
	if err != nil {
		fmt.Println(err.Error())
		c.JSON(500, `{"error":"sql error"}`)
		return
	...
	for i, v := range game.Players {
		...
		if v.Id != nil {
			...
			if numplayers <= 1 {
				...
				interpolatedscore = (1 - (float32(i) / float32(numplayers-1)))
			}
			...
			_, err = gamestmt.Exec(game.Id, v.Id, i, numplayers)
			if err != nil {
				fmt.Println(err.Error())
				c.JSON(500, `{"error":"error updating db"}`)
				return
